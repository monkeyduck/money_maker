...
from strategy import get_macd_val
from entity import Coin, Indicator, DealEntity
from config_loose import okFuture, vol_1m_line, vol_3s_line, vol_1m_bal, vol_3s_bal, incr_5m_rate, incr_1m_rate
import time
import json
...
def on_message(ws, message):
    ...
    if int(ts) - int(last_5min_macd_ts/1000) >= 300:
        ...
        macd_5min, diff_5min, dea_5min, last_5min_macd_ts = get_macd_val(okFuture, coin.name + "_usd", time_type, "5min", 300)
            ...
            if more == 1:
                ...
                if price_1m_change >= incr_1m_rate and price_5m_change >= incr_5m_rate:
                    if ind_3s.bid_vol >= vol_3s_bal * ind_3s.ask_vol and ind_1min.bid_vol >= vol_1m_bal * ind_1min.ask_vol:
                        if buyin_more(okFuture, coin.name, time_type, latest_price):
                            more = 1
                    ...
                    if ind_3s.ask_vol >= vol_3s_bal * ind_3s.bid_vol and ind_1min.ask_vol >= vol_1m_bal * ind_1min.bid_vol:
                        if buyin_less(okFuture, coin.name, time_type, latest_price):
                            less = 1
