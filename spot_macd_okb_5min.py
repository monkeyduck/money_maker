    ...
    while True:
        try:
            ...
            for i in range(len(order_id_queue)):
                ...
                print('now timestamp: %d, order created time: %d' % (int(ts), string2timestamp(order_info['created_at']) + 8*3600))
                if status == 'filled':
                ...
                elif int(ts) - (string2timestamp(order_info['created_at']) + 8 * 3600) >= 3:
                    print('撤单重挂')
                    try:
                        revoke_ret = spotAPI.revoke_order(instrument_id, old_order_id)
                        print('撤单结果: %s' % revoke_ret)
                    except Exception as e:
                        print(repr(e))
                        traceback.print_exc()
                        del_list.append(old_order_id)
                        continue
                    print('当前行情: %s' % spotAPI.get_specific_ticker(instrument_id))
                    if side == 'buy':
            ...
            if int(ts) - last_minute_ts > 60:
                ...
                if more == 1 and (new_macd < 0 or (check_trend(list(df['macd'])) == 'down' and new_macd < buy_thread)):
                    sell_price = float(ret['best_ask']) - 0.0005
                    order_id = sell_all_position(spotAPI, instrument_id, sell_price)
                    if order_id:
                        order_id_queue.append(order_id)

                elif more == 0 and last_macd <= buy_thread < new_macd:
                    buy_price = float(ret['best_bid']) + 0.0005
                    order_id = buy_all_position(spotAPI, instrument_id, buy_price)
                    if order_id:
                        order_id_queue.append(order_id)
