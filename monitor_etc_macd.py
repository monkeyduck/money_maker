...
def check_macd_trend(macd):
    if macd[-1] > macd[-2] > macd[-3] > macd[-4]:
        return 'up'
    elif macd[-1] < macd[-2] < macd[-3] < macd[-4]:
        return 'down'
    else:
        return False
    ...
    while True:
        try:
            ...
            if int(ts) - last_minute_ts > 60:
                ...
                buy_price = ret['buy']
                sell_price = ret['sell']
                if more == 1 and last_macd > 0 > new_macd:
                    order_id = sell_more_price(okFuture, coin.name, time_type, sell_price)
                    if order_id:
                        order_id_queue.append(order_id)

                elif less == 1 and new_macd > 0 > last_macd:
                    order_id = sell_less_price(okFuture, coin.name, time_type, buy_price)
                    if order_id:
                        order_id_queue.append(order_id)

                if more == 0 and last_macd < 0.0015 < new_macd:
                    order_id = buyin_more_price(okFuture, coin.name, time_type, buy_price)
                    if order_id:
                        order_id_queue.append(order_id)
                        print("挂单开多, order_id: %s, 价格: %.3f" % (str(order_id), buy_price))
                elif less == 0 and last_macd > -0.0015 > new_macd:
                ...
                if first:
                    ...
                    for i in range(0, len(diff)):
                        print("5min macd: %.6f, diff: %.6f, dea: %.6f, macd_time: %s" % (
                        2 * (diff[i] - dea[i]), diff[i], dea[i], timestamp2string(timestamp[i])))
                print("5min  macd: ", new_macd, diff[-1], dea[-1], timestamp2string(timestamp[-1]), timestamp2string(ts))
                print("15min macd: ", new_macd_15, diff_15[-1], dea_15[-1], timestamp2string(timestamp_15[-1]), timestamp2string(ts))
