...
def check_do_future_more(price_3m_change, price_1m_change, price_10s_change):
    if ind_1min.vol > 300000 and ind_3m.vol > 400000 and -0.2 < price_1m_change \
            and price_3m_change < -0.5 and price_10s_change >= 0.2 and new_macd < 0:
        return True
...
def check_do_future_more(price_3m_change, price_1m_change, price_10s_change):
    if ind_1min.vol > 300000 and ind_1min.ask_vol * 2 < ind_1min.bid_vol \
            and ind_3m.vol > 400000 and ind_3m.ask_vol * 1.3 < ind_3m.bid_vol \
            and price_3m_change > price_1m_change > 0.3 and price_10s_change >= 0.1 \
            and new_macd > 0:
        return True
...
def on_message(ws, message):
    ...
    for json_data in json_message['data']:
        ...
        deal_entity = DealEntity(json_data['trade_id'], latest_price, round(float(json_data['size']), 3), ts, json_data['side'])
        ...
        price_5m_ago_change = cal_rate(avg_3s_price, deque_5m[0].price)
        print('5m change: %.3f', price_5m_ago_change)
