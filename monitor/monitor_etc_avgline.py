...
from trade import buyin_less, buyin_more, ensure_buyin_less, sell_more, sell_less, \
    ensure_buyin_more, okFuture, cancel_uncompleted_order, gen_orders_data, send_email, buyin_less_price, buyin_more_price
from strategy import get_macd
from entity import Coin, Indicator, DealEntity
from config_avg import vol_1m_line, vol_3s_line, vol_1m_bal, vol_3s_bal, incr_5m_rate, incr_1m_rate
import time
import json
...
prev_5min_price_more_15min = False
prev_5min_price_less_15min = False
    ...
    for each_message in jmessage:
        for jdata in each_message['data']:
            ...
            if more == 1:
                ...
                if buyin_more_price(coin.name, time_type, latest_price - 0.002):
                    more = 1
                    count = 0
                ...
                if buyin_less_price(coin.name, time_type, latest_price + 0.002):
                    less = 1
                    count = 0
                    buy_price = latest_price + 0.002
...
def ensure_sell_more(coin_name, time_type, lever_rate=20):
    ...
    ts = time.time()
    now_time = timestamp2string(ts)
    info = u'做多卖出成功！！！卖出价格：' + str(latest_price) + u', 收益: ' + str(round(latest_price - buy_price), 3) \
           + ', ' + now_time
